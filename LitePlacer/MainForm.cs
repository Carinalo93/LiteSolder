...
namespace LitePlacer
{
    ...
    public partial class FormMain : Form
    {
        ...
        private void FormMain_Shown(object sender, EventArgs e)
        {
            ...
            //A axis place and dispense mode - write value to textbox/Button
            m4place_radioButton.Checked = Setting.m4place_radioButton;
            m4dispense_radioButton.Checked = Setting.m4dispense_radioButton;
            m4deg09_place_radioButton.Checked = Setting.m4deg09_place_radioButton;
            m4deg18_place_radioButton.Checked = Setting.m4deg18_place_radioButton;
            m4deg09_dispense_radioButton.Checked = Setting.m4deg09_dispense_radioButton;
            m4deg18_dispense_radioButton.Checked = Setting.m4deg18_dispense_radioButton;


            ajm_place_maskedTextBox.Text = Setting.ajm_place.ToString("", CultureInfo.InvariantCulture);
            ajm_dispense_maskedTextBox.Text = Setting.ajm_dispense.ToString("", CultureInfo.InvariantCulture);
            avm_place_maskedTextBox.Text = Setting.avm_place.ToString("", CultureInfo.InvariantCulture);
            avm_dispense_maskedTextBox.Text = Setting.avm_dispense.ToString("", CultureInfo.InvariantCulture);
            m4_place_maskedTextBox.Text = Setting.m4_place.ToString("", CultureInfo.InvariantCulture);
            m4_dispense_maskedTextBox.Text = Setting.m4_dispense.ToString("", CultureInfo.InvariantCulture);
            tr4_place_textBox.Text = Setting.tr4_place.ToString("", CultureInfo.InvariantCulture);
            ...
            SwitchingPointHeight_textBox.Text = Setting.switchingPointHeight.ToString("0.00", CultureInfo.InvariantCulture);
            NozOffsetX_textBox.Text = Setting.dispenseNozOffsetX.ToString("0.00", CultureInfo.InvariantCulture);
            NozOffsetY_textBox.Text = Setting.dispenseNozOffsetY.ToString("0.00", CultureInfo.InvariantCulture);
        ...
        */
        ...
        private bool MechanicalHoming_m()
        {
            ...
            // DisplayText("move A");
            if (m4place_radioButton.Checked)
            {
                if (!CNC_A_m(0))
                {
                    return false;
        ...
        private void tabPageSetupCameras_Begin()
        {
            ...
            zCalibrationInstructions.Visible = false;
            ClearEditTargets();
        ...
        */
        ...
        private bool ControlBoardJustConnected()
        {
            ...
            if (Cnc.Controlboard == CNC.ControlBoardType.TinyG)
            {
                ...
                //Laden der Dispense bzw. Place Einstellungen
                m4dispense_radioButton_Click(null, null);
                m4place_radioButton_Click(null, null);
        ...
        private void avm_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == '\r')
            {
                CNC_Write_m("{\"avm\":" + avm_maskedTextBox.Text + "}");
                Thread.Sleep(50);
                CNC_Write_m("{\"afr\":" + avm_maskedTextBox.Text + "}");
                Thread.Sleep(50);
...
*/
        ...
        private void m4place_radioButton_Click(object sender, EventArgs e)
        {

            if (m4place_radioButton.Checked)
            {
                DialogResult switched = ShowMessageBox("mechanical switch on state: Place?","Switched Motor A?",
                MessageBoxButtons.OK);


                Setting.m4place_radioButton = m4place_radioButton.Checked;
                Setting.m4dispense_radioButton = false;

                DisplayText("Place-Mode selected");
                PlaceOne_button.Enabled = true;
                PlaceAll_button.Enabled = true;
                DoDispense_button.Enabled = false;

                CNC_Write_m("{\"4po\":" + 0 + "}");     //normal polarity
                CNC_Write_m("{\"4pm\":" + 2 + "}");     //power management - 2 in cycle

#if (TINYG_SHORTUNITS)
                CNC_Write_m("{\"ajm\":" + Setting.ajm_place + "}");         //send ajm to TinyG
#else
                CNC_Write_m("{\"ajm\":" + Setting.ajm_place + "000000}");
#endif
                Thread.Sleep(50);

                CNC_Write_m("{\"avm\":" + Setting.avm_place + "}");      //send avm/afr to TinyG
                Thread.Sleep(50);

                CNC_Write_m("{\"afr\":" + Setting.avm_place + "}");
                Thread.Sleep(50);

                CNC_Write_m("{\"4mi\":" + Setting.m4_place + "}");          //send 4mi to TinyG
                Thread.Sleep(50);

                CNC_Write_m("{\"4tr\":" + Setting.tr4_place + "}");         //send 4tr to TinyG
                Thread.Sleep(50);

                if (Setting.m4deg09_place_radioButton == true)              //send 4sa to TinyG
                {
                    CNC_Write_m("{\"4sa\":0.9}");
                    Thread.Sleep(50);

                }
                if (Setting.m4deg18_place_radioButton == true)
                {
                    CNC_Write_m("{\"4sa\":1.8}");
                    Thread.Sleep(50);
                }
            ...
            {
                PlaceOne_button.Enabled = false;
                PlaceAll_button.Enabled = false;
                DoDispense_button.Enabled = true;
        ...
        private void m4dispense_radioButton_Click(object sender, EventArgs e)
        {
            if (m4dispense_radioButton.Checked)
            {
                DialogResult switched = ShowMessageBox("mechanical switch on state: Dispense?", "Switched Motor A?",
                MessageBoxButtons.OK);

                Setting.m4dispense_radioButton = m4dispense_radioButton.Checked;
                Setting.m4place_radioButton = false;

                DisplayText("Dispense-Mode selected");
                PlaceOne_button.Enabled = false;
                PlaceAll_button.Enabled = false;
                DoDispense_button.Enabled = true;

                CNC_Write_m("{\"4po\":" + 1 + "}");     //reverse polarity
                CNC_Write_m("{\"4pm\":" + 3 + "}");     //power management - 3 when moving

#if (TINYG_SHORTUNITS)
                CNC_Write_m("{\"ajm\":" + Setting.ajm_dispense + "}");          //send to TinyG
#else
                CNC_Write_m("{\"ajm\":" + Setting.ajm_dispense + "000000}");
#endif
                Thread.Sleep(50);

                CNC_Write_m("{\"avm\":" + Setting.avm_dispense + "}");      //send avm/afr to TinyG
                Thread.Sleep(50);

                CNC_Write_m("{\"afr\":" + Setting.avm_dispense + "}");
                Thread.Sleep(50);

                CNC_Write_m("{\"4mi\":" + Setting.m4_dispense + "}");          //send 4mi to TinyG
                Thread.Sleep(50);

                CNC_Write_m("{\"4tr\":" + Setting.tr4_dispense + "}");         //send 4tr to TinyG
                Thread.Sleep(50);

                if (Setting.m4deg09_dispense_radioButton == true)              //send 4sa to TinyG
                {
                ...
                if (Setting.m4deg18_dispense_radioButton == true)
                {
                    CNC_Write_m("{\"4sa\":1.8}");
                    Thread.Sleep(50);
                }
        ...
        private void ajm_place_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            {
                double val;
                ajm_place_maskedTextBox.ForeColor = Color.Red;
                if (e.KeyChar == '\r')
                {
                    if (double.TryParse(ajm_place_maskedTextBox.Text.Replace(',', '.'), out val))
                    {
                        Setting.ajm_place = val;        //save in Settings

#if (TINYG_SHORTUNITS)
                        CNC_Write_m("{\"ajm\":" + val + "}");
#else
                CNC_Write_m("{\"ajm\":" + val + "000000}");
#endif
                    ...
                    ajm_place_maskedTextBox.ForeColor = Color.Black;
                }
        ...
        private void ajm_dispense_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            {
                double val;
                ajm_dispense_maskedTextBox.ForeColor = Color.Red;
                if (e.KeyChar == '\r')
                {
                    if (double.TryParse(ajm_dispense_maskedTextBox.Text.Replace(',', '.'), out val))
                    {
                        Setting.ajm_dispense = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeichert
                ...
                CNC_Write_m("{\"ajm\":" + val + "000000}");
                    ...
                    ajm_dispense_maskedTextBox.ForeColor = Color.Black;
                }
        ...
        private void avm_place_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            avm_place_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                double val;
                if (double.TryParse(avm_place_maskedTextBox.Text.Replace(',', '.'), out val))
                {
                    Setting.avm_place = val;
                    ...
                    avm_place_maskedTextBox.ForeColor = Color.Black;

                    int peek = Convert.ToInt32(val);
                    Setting.CNC_AspeedMax = peek;
                }
            }
        ...
        private void avm_dispense_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            avm_dispense_maskedTextBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                double val;
                if (double.TryParse(avm_dispense_maskedTextBox.Text.Replace(',', '.'), out val))
                {
                    Setting.avm_dispense = val;
                    ...
                    avm_dispense_maskedTextBox.ForeColor = Color.Black;
        ...
        private void m4_place_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            if (double.TryParse(m4_place_maskedTextBox.Text.Replace(',', '.'), out val))
            {
                Setting.m4_place = val;
                
                Microstep_KeyPress(m4_place_maskedTextBox, 4, e);   
            }
        }

        private void m4_dispense_maskedTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            if (double.TryParse(m4_dispense_maskedTextBox.Text.Replace(',', '.'), out val))
            {
                Setting.m4_dispense = val;

                Microstep_KeyPress(m4_dispense_maskedTextBox, 4, e);
            }
        }

        private void m4deg09_place_radioButton_Click(object sender, EventArgs e)
        {
 
            Setting.m4deg09_place_radioButton = m4deg09_place_radioButton.Checked;
            Setting.m4deg18_place_radioButton = false;

            if (m4deg09_place_radioButton.Checked)
            {
        ...
        private void m4deg18_place_radioButton_Click(object sender, EventArgs e)
        {
            Setting.m4deg18_place_radioButton = m4deg18_place_radioButton.Checked;
            Setting.m4deg09_place_radioButton = false;
        ...
        private void m4deg09_dispense_radioButton_Click(object sender, EventArgs e)
        {
            Setting.m4deg09_dispense_radioButton = m4deg09_dispense_radioButton.Checked;
            Setting.m4deg18_dispense_radioButton = false;

            if (m4deg09_dispense_radioButton.Checked)
            {
        ...
        private void m4deg18_dispense_radioButton_Click(object sender, EventArgs e)
        {
            Setting.m4deg18_dispense_radioButton = m4deg18_dispense_radioButton.Checked;
            Setting.m4deg09_dispense_radioButton = false;
        ...
        private void tr4_place_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            
            tr4_place_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(tr4_place_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.tr4_place = val;
                    CNC_Write_m("{\"4tr\":" + val + "}");
                    Thread.Sleep(50);
                    tr4_place_textBox.ForeColor = Color.Black;
                }
            }

        }

        private void tr4_dispense_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            tr4_dispense_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(tr4_dispense_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.tr4_dispense = val;
                    CNC_Write_m("{\"4tr\":" + val + "}");
                    Thread.Sleep(50);
                    tr4_dispense_textBox.ForeColor = Color.Black;
                }
            }
        }

        private class PcbPad
        {
            public string Type { get; set; }  // Round, Square, etc
            public double X_nominal { get; set; }  //Position in PCB Coordinates
            public double Y_nominal { get; set; }
            public double W_nominal { get; set; }
            public double X_size { get; set; }  //Sizes with centered origin
            public double Y_size { get; set; }
            public double X_machine { get; set; }
            public double Y_machine { get; set; }
            public double W_machine { get; set; }

        }
        private class FootprintType
        {
            public int Number { get; set; }  // Round, Square, e
            public double X_size { get; set; }  //Sizes with centered origin
            public double Y_size { get; set; }
            public string Type { get; set; }
        }

        List<PcbPad> currentPcbPads = new List<PcbPad>();
        List<PcbPad> padsformeasure = new List<PcbPad>();
        private void LoadGBR_Pos()    //Einlesen von GBR Files
        {
            var filePath = string.Empty;
            var fileName = string.Empty;

            currentPcbPads.Clear();

            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                openFileDialog.InitialDirectory = "c:\\";
                openFileDialog.Filter = ".gbr files (*.gbr)|*.gbr|All files (*.*)|*.*";
                ...
                {
                    DisplayText("Loaded nothing!");
                    SolderFile_Label.Text = "---";
                    return;
                }
            ...
            TextReader sr_gbr = new StreamReader(filePath);
            SolderFile_Label.Text = fileName;

            string line;

            List<FootprintType> Footprints = new List<FootprintType>();
            List<PcbPad> PadsOnPcB = new List<PcbPad>();
            int lastFootprintType = 0;
            double decimalX = -1.0;
            double decimalY = -1.0;

            // read lines of text
            while ((line = sr_gbr.ReadLine()) != null)
            {
                if (line.StartsWith("%FSLAX")) //%FSLAX34Y34*%
                {
                    decimalX = Math.Pow(10, Int32.Parse(line.Substring(7,1)));
                    decimalY = Math.Pow(10, Int32.Parse(line.Substring(10,1)));
                }
                if (line.StartsWith("%MOMM"))
                {
                    DisplayText(".gbr file in correct units.");
                }
                if (line.StartsWith("%MOIN"))
                {
                    DisplayText(".gbr file in incorrect units, use mm!");
                    SolderFile_Label.Text = "---";
                    currentPcbPads.Clear();
                    sr_gbr.Close();
                    return;
                ...
                {
                    FootprintType currentFootprint = new FootprintType { };
                    if (line.IndexOf("R") == 6)
                    {

                        currentFootprint.Number = Int32.Parse(line.Substring(4, 2));
                        currentFootprint.Type = "R";
                        currentFootprint.X_size = Double.Parse(line.Substring(line.IndexOf(",") + 1, (line.IndexOf("X") - 1) - (line.IndexOf(",") + 1)));
                        currentFootprint.Y_size = Double.Parse(line.Substring(line.IndexOf("X") + 1, (line.IndexOf("*") - 1) - (line.IndexOf("X") + 1)));
                    }
                    if (line.IndexOf("C") == 6)
                    {

                        currentFootprint.Number = Int32.Parse(line.Substring(4, 2));
                        currentFootprint.Type = "C";
                        currentFootprint.X_size = Double.Parse(line.Substring(line.IndexOf(",") + 1, (line.IndexOf("*") - 1) - (line.IndexOf(",") + 1)));
                        currentFootprint.Y_size = Double.Parse(line.Substring(line.IndexOf(",") + 1, (line.IndexOf("*") - 1) - (line.IndexOf(",") + 1)));
                    }

                    Footprints.Add(currentFootprint);
                }

                if (line.StartsWith("D"))
                {
                    lastFootprintType = Int32.Parse(line.Substring(1, 2));
                }

                if (line.StartsWith("X"))
                {
                    if(decimalX == -1 || decimalY == -1)
                    {
                        DisplayText($"Can't read the FSLA command for decimals. Is the file correct?");
                        SolderFile_Label.Text = "---";
                        currentPcbPads.Clear();
                        sr_gbr.Close();
                        return;
                    ...
                    // Read XY coordinates
                    PcbPad SinglePad = new PcbPad { };

                    string pos_string = line.Substring(0, line.Length - 4);
                    double x = (Int32.Parse(pos_string.Substring(1, pos_string.IndexOf('Y') - 1))) / decimalX;

                    SinglePad.X_nominal = x;
                    
                    string val = pos_string.Substring(pos_string.IndexOf('Y') + 1, pos_string.Length - (pos_string.IndexOf('Y') + 1));
                    double y = (Int32.Parse(val)) / decimalY;

                    SinglePad.Y_nominal = y;

                                        //Connect coordinates to the right footprint
                    SinglePad.X_size = Footprints[lastFootprintType - 10].X_size;
                    SinglePad.Y_size = Footprints[lastFootprintType - 10].Y_size;

                    PadsOnPcB.Add(SinglePad);
            ...
            int count = 0;
            for (count = 0; count < PadsOnPcB.Count; count++)
            {
                DisplayText($"Position: {String.Format("{0,3:N0}", count)}  X:{String.Format("{0,7:N3}", PadsOnPcB[count].X_nominal)} Y:{String.Format("{0,7:N3}", PadsOnPcB[count].Y_nominal)} " +
                    $"  X-Size:{String.Format("{0,7:N3}", PadsOnPcB[count].X_size)} Y-Size:{String.Format("{0,7:N3}", PadsOnPcB[count].Y_size)}");
            }
            DisplayText($"Number of Positions: {count}");
            currentPcbPads = PadsOnPcB;


            if (PadsOnPcB.Count == 0)
            {
                DisplayText($"No Pads found. Please check the .gbr - File!");
            }
        }
        ...
        private bool DoSolderpasteDispense(List<PcbPad> PadsOnPcB, double NozToCamX, double NozToCamY)
        {
            ...
            for (int i = 0; i < PadsOnPcB.Count; i++)
            {
                ...
                CNC_XY_m(PadsOnPcB[i].X_machine + NozToCamX, PadsOnPcB[i].Y_machine + NozToCamY); // move to pad
                if (Cnc.ErrorState)
                {
                    ZGuardOn();
                    return false;
        ...
        private void DoDispense_button_Click(object sender, EventArgs e)
        {
            if(currentPcbPads.Count == 0)
            {
        ...
        private void DispenseSpeed_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            DispenseSpeed_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(DispenseSpeed_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.B_dispense_speed = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    DispenseSpeed_textBox.ForeColor = Color.Black;
                }
            }
        }

        private void DispenseValue_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            DispenseValue_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(DispenseValue_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispense_value = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    DispenseValue_textBox.ForeColor = Color.Black;
                }
            }
        }

        private void DispenseRetract_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            DispenseRetract_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(DispenseRetract_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispense_retract = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    DispenseRetract_textBox.ForeColor = Color.Black;
                }
            }
        }

        private void NoztoPCB_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            NoztoPCB_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(NoztoPCB_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispense_NozPcbZero = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    NoztoPCB_textBox.ForeColor = Color.Black;
                    DisplayText("Dispense nozzle to PCB distance:" + Setting.dispense_NozPcbZero.ToString());
                }
            }
            
        }

        private void DispenseOffset_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            DispenseOffset_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(DispenseOffset_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispense_Offset = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    DispenseOffset_textBox.ForeColor = Color.Black;
                }
            }
        }
        // BuildMachineCoordinateData_m():
        private bool BuildMachineCoordinateDataDispense_m()
        {
            double X_nom = 0;
            double Y_nom = 0;
            if (Setting.Placement_SkipMeasurements)
            {
                if(currentPcbPads.Count == 0)
                {
                    DisplayText($"Found no PCB pads, do you load the .gbr file?");
                    return false;
                }
                for(int i = 0; i < currentPcbPads.Count; i++)
                {
                    // Cad data is validated.
                    X_nom = currentPcbPads[i].X_nominal;
                    Y_nom = currentPcbPads[i].Y_nominal;
                    X_nom += Setting.General_JigOffsetX;
                    Y_nom += Setting.General_JigOffsetY;
                    currentPcbPads[i].X_machine = X_nom;
                    currentPcbPads[i].Y_machine = Y_nom;
                }
                for (int count = 0; count < currentPcbPads.Count; count++)
                {
                    DisplayText($"Nominal position: {count} X: {String.Format("{0,7:N3}", currentPcbPads[count].X_machine)} Y: {String.Format("{0,7:N3}", currentPcbPads[count].Y_machine)} ");
                }
                DisplayText($"Skipped fiducial measurement! - uncheck skip the measurements checkbox in RunJob Tab");
                return true;
            };
            ...
            for(int i = 0; i < currentPcbPads.Count; i++)
            {
                // build a point from dispense data values
                Loc.X = currentPcbPads[i].X_nominal;
                Loc.Y = currentPcbPads[i].Y_nominal;
                Loc.W = 1;
                // transform it
                Loc = transform.TransformPoint(Loc);
                Loc = Loc.NormalizeHomogeneous();
                // store calculated location values
                currentPcbPads[i].X_machine = Loc.X;
                currentPcbPads[i].Y_machine = Loc.Y;
            ...
            for (int count = 0; count <currentPcbPads.Count; count++)
            {
                DisplayText($"Measured Position: {count} X: {String.Format("{0,7:N3}", currentPcbPads[count].X_machine)} Y: {String.Format("{0,7:N3}", currentPcbPads[count].Y_machine)} ");
            }
            DisplayText($"Measured ficiduals on PCB succesful!");
            return true;
        }// end BuildMachineCoordinateDataDispense_m

        private void MeasureDispenseNozzleXY()
        {
            //check for valid measurement settings
            if(Math.Abs(Setting.dispenseNozOffsetX) <  10 && Math.Abs(Setting.dispenseNozOffsetY) < 10)
            {
                DisplayText($"Invalid data for XY measurement. You need a not perfect measurement between the camera and the dispense nozzle.");
                return;
            }
            padsformeasure.Clear();
            
            //Create Pads for Measure
            for (int i = 0; i < 10; i++)
            {
                PcbPad SinglePad1 = new PcbPad { };
                SinglePad1.X_machine = 30 + i;
                SinglePad1.Y_machine = 20;
                padsformeasure.Add(SinglePad1);

                PcbPad SinglePad2 = new PcbPad { };
                SinglePad2.X_machine = 30 + i;
                SinglePad2.Y_machine = 19;
                padsformeasure.Add(SinglePad2);

                PcbPad SinglePad3 = new PcbPad { };
                SinglePad3.X_machine = 30 + i;
                SinglePad3.Y_machine = 18;
                padsformeasure.Add(SinglePad3);
            }
            //One last pad for measuring
            PcbPad SinglePad = new PcbPad { };
            SinglePad.X_machine = 30 + 20;
            SinglePad.Y_machine = 18;
            padsformeasure.Add(SinglePad);
            ...
            while ((Successes < 7) && (Tries < 30));
            if (Tries >= 30)
            {
                DisplayText("Optical measurement failed, 30 tries did not give 7 results.");
                return;
            }
            Xlist.Sort();
            Ylist.Sort();
        ...
        private void DispensePCB_measure_Click(object sender, EventArgs e)
        {
            BuildMachineCoordinateDataDispense_m();
        }
        
        private void Z_calibration_button_Click(object sender, EventArgs e)
        {
                ...
                {
                    case 0:
                        DialogResult dialog = ShowMessageBox("Z-calibration switch placed on PCB? measured switching point height and registered in box (rather too small than to big)?", "Z-Calibration",
                        MessageBoxButtons.OK);
                        SetNozzleOffset_stage = 1;
                        Z_calibration_button.Text = "Next";
                        zCalibrationInstructions.Visible = true;
                        zCalibrationInstructions.Text = "Jog Nozzle with x/y movements to z-calibration switch, then click \"Next\"";
                        break;
                    case 1:
                        SetNozzleOffset_stage = 2;
                        CNC_Z_m(0);
                        Z_calibration_button.Text = "Next";
                        zCalibrationInstructions.Visible = true;
                        zCalibrationInstructions.Text = "Jog Nozzle with small z movements (+0.1) down to z-calibration switch until you hear it \"Click\", then click \"Next\"";
                        break;
                    case 2:
                        SetNozzleOffset_stage = 0;
                        double val = 0;
                        val = Cnc.CurrentZ;                         //z-Position after calibrating height
                        val = val + Setting.switchingPointHeight;   //on PCB zero
                        Setting.dispense_NozPcbZero = val;          //safe value to Dispense Nozzle to PCB
                        NoztoPCB_textBox.Text = Setting.dispense_NozPcbZero.ToString("0.00", CultureInfo.InvariantCulture);
                        zCalibrationInstructions.Visible = false;
                        zCalibrationInstructions.Text = " ";
                        Z_calibration_button.Text = "Measure Z";
                        DialogResult dialog2 = ShowMessageBox("Value is safed in Dispense Nozzle to PCB", "Z-Calibration",
                        MessageBoxButtons.OK);
                        CNC_Z_m(0.0);
                        ZGuardOn();
                        break;
        ...
        private void SwitchingPointHeight_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            SwitchingPointHeight_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(SwitchingPointHeight_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.switchingPointHeight = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    SwitchingPointHeight_textBox.ForeColor = Color.Black;
                }
            }
        }

        private void ButtonMeasureXY_Click(object sender, EventArgs e)
        {
            MeasureDispenseNozzleXY();
        }

        private void NozOffsetX_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            NozOffsetX_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(NozOffsetX_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispenseNozOffsetX = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    NozOffsetX_textBox.ForeColor = Color.Black;
                }
            }
        }

        private void NozOffsetY_textBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            double val;
            NozOffsetY_textBox.ForeColor = Color.Red;
            if (e.KeyChar == '\r')
            {
                if (double.TryParse(NozOffsetY_textBox.Text.Replace(',', '.'), out val))
                {
                    Setting.dispenseNozOffsetY = val; //eingegebener Parameter wird in Setting-//Variable geschrieben und gespeicher
                    NozOffsetY_textBox.ForeColor = Color.Black;
                }
            }
        }
